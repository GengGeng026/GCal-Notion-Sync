    ...
    if end is None:
        ...
        if start is not None and start_end is not None:
            ...
            if StartEnd_to_Overwrite_All:
                ...
                # Ensure correctness when handling timezone information
                start_end_naive = start.replace(tzinfo=None) if isinstance(start, datetime) and start.tzinfo is not None else start
                # 如果 start 是 datetime 对象且包含时区信息，则转换为无时区信息的 datetime 对象
                start_naive = start.replace(tzinfo=None) if isinstance(start, datetime) and start.tzinfo is not None else start
                # 使用 start 的值覆盖 start_end
                start_end = start_naive

                # When updating 'end', ensure the time is not incorrectly set to 00:00
                if start_end_naive != start_naive or StartEnd_to_Overwrite_All == False:
                    original_start_end = start_end
                    original_start = start
                    original_end = end
                    ...
                    if start_end_prop:
                        ...
                        # Check and preserve the time part
                    ...
                    if start != original_start or end != original_end:
                        ...
                        # Update the page in the Notion database
                        with lock:
                            start_end_list = [parse(start_end_prop['start']) if start_end_prop['start'] is not None else None,
                                            parse(start_end_prop['end']) if start_end_prop['end'] is not None else None]
