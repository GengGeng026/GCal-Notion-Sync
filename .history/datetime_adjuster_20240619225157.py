            ...
            if StartEnd_to_Overwrite_All == True:
                ...
                if start_end_prop:  # Check if start_end_prop is not None and not empty
                    ...
                    with lock:
                        ...
                        # Determine whether to update 'end' or keep it as None
                        if 'end' in start_end_prop and start_end_prop.get('end') is not None:
                            end_date = start_end_prop['end']
                            if not isinstance(end_date, datetime):
                                end_date = parse(end_date)
                            # Only add end_date to start_end_list if it needs to be updated

                        # If 'end' is not in start_end_prop, it implies we might want to keep it as None or unchanged
                        # Uncomment the next line if you wish to explicitly set it to None when it's absent
                        # start_end_list.append(None)
                        # No operation here means end_date remains unchanged or None based on previous logic
