...
def dynamic_counter_indicator(stop_event, progress):
    total_parts = 5
    while not stop_event.is_set():
        filled_length = int(50 * progress.value // total_parts)
        bar = '█' * filled_length + '-' * (50 - filled_length)
        sys.stdout.write(f'\rProgress: |{bar}| {int((progress.value / total_parts) * 100)}% Complete')
        sys.stdout.flush()
        time.sleep(0.5)  # 更新間隔
    print()  # 確保在進度條下方輸出

def complex_task(progress, part):
    # 模擬複雜任務
    time.sleep(2)  # 假設這是一個耗時的操作
    progress.value = part  # 更新進度值

if __name__ == "__main__":
    stop_event = Event()
    progress = Value('i', 0)  # 'i' 表示整數
    p = Process(target=dynamic_counter_indicator, args=(stop_event, progress))
    p.start()

    try:
        for part in range(1, 6):  # Part 1 到 Part 5
            complex_task(progress, part)
    finally:
        stop_event.set()
        p.join()
        print("腳本結束")