...
def format_gradient(length, filled_length, gradient_state):
    formatted_bar = ""
    for i in range(length):
        if i < filled_length:
            # 根据渐变状态和位置应用不同的亮度
            brightness = (i + gradient_state) % filled_length
            if brightness < filled_length / 3:
                formatted_bar += f"{DIM}━{RESET}"
            elif brightness < (2 * filled_length / 3):
                formatted_bar += f"━"
            else:
                formatted_bar += f"{BRIGHT}━{RESET}"
        else:
            formatted_bar += "━"
...
class ProgressThread(threading.Thread):
    ...
    def run(self):
        while not self.stop_event.is_set():
            ...
            bar = format_gradient(bar_length, filled_length, self.gradient_state)
            sys.stdout.write(f'\r   {bar} {percent_complete:.1f}%')
            sys.stdout.flush()
            time.sleep(0.1)  # 更新间隔
            self.gradient_state += 1  # 更新渐变状态
        print()  # 确保在进度条下方输出
...
def complex_task(progress_thread, size):
    # 模擬複雜任務
    for _ in range(100):  # 分100次完成，每次增加1%
        time.sleep(0.02)  # 假設這是一個較短的操作
        progress_thread.update_progress(size / 100)  # 更新進度值

if __name__ == "__main__":
    total_size = 1.8 * 1024 * 1024  # 假設總大小為1.8MB
    progress_thread = ProgressThread(total_size)
    progress_thread.start()

    try:
        complex_task(progress_thread, total_size)
    finally:
