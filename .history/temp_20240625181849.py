...
class ProgressThread(threading.Thread):
    ...
    def run(self):
        while not self.stop_event.is_set():
            ...
            sys.stdout.write(f'\r{animate_text_wave}   {formatted_bar} {percent_complete:.1f}%')
            sys.stdout.flush()
...
def update_progress(size, progress_thread, updates, sleep_time=0.02):
    for _ in range(updates):
        # Clear the screen and redraw the animation text before updating the progress bar
        print("\r\033[K", end="")  # Clear the current line
        animate_text_wave("current_phase", repeat=1)  # Redraw the animation text, repeat=1 just for example
        progress_thread.update_progress(0.1)  # Update the progress bar
        time.sleep(sleep_time)
