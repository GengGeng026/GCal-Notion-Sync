...
class ProgressThread(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            # 在進度百分比前加上旋轉的指示器
            sys.stdout.write(f'\r{self.animation[animation_index % len(self.animation)]} Progress: {percent_complete}% Complete')
            sys.stdout.flush()
            time.sleep(0.1)  # 更新間隔較短，使動畫更流暢
            animation_index += 1
        print()  # 確保在進度條下方輸出
