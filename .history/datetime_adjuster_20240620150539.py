...
def update_page_properties(notion, page, Start_Notion_Name, End_Notion_Name, Date_Notion_Name, start, end, start_end, as_date=False, single_date=False, update_all=True, keep_midnight=False, remove_start_end_midnight=False, keep_start_midnight=False, keep_end_midnight=False, StartEnd_to_Overwrite_All=False):
    ...
    # 根据 start 和 end 直接设置 StartEnd 的值
...
with ThreadPoolExecutor(max_workers=3) as executor:
    ...
    def format_datetime_conditionally(original, new, extra, formatted_colon, formatted_right_arrow, formatted_plain_none):
        """
        根据给定的条件格式化日期时间字符串。
        """
        formatted = ''
        arrow_space = '  ' if original is None and (new is not None or extra is not None) else ' '
        none_space = ' ' if new is None else ''

        if original is None:
            formatted += formatted_plain_none + arrow_space
        elif new != original:
            formatted += DateTimeIntoNotionFormat(new, date_only=False, time_format='24') + none_space

        if original is None and new is not None:
            formatted += formatted_right_arrow + none_space

        if new is None:
            formatted += formatted_plain_none
        elif new != original:
            formatted += DateTimeIntoNotionFormat(new, date_only=False, time_format='24')
    ...
    def process_pages_filled_details(pages_filled_details):
        ...
        for updated_details in pages_filled_details:
            ...
            # 使用新的函数来简化原始的打印逻辑
            print(f"{formatted_start}    {formatted_colon}  {format_datetime_conditionally(original_start, start_new, extra, formatted_colon, formatted_right_arrow, formatted_plain_none)}")
            print(f"{formatted_end}      {formatted_colon}  {format_datetime_conditionally(original_end, end_new, extra, formatted_colon, formatted_right_arrow, formatted_plain_none)}")
