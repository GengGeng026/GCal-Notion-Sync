...
def process_pages_condition_A(page, counts, details, lock, processed_pages, return_values):
    ...
    if end is None:
        ...
        if start is not None and start_end is not None:
            ...
            # Additional logic to update StartEnd based on Start
            if start_end is not None and start is not None and not StartEnd_to_Overwrite_All:
                # Ensure StartEnd is updated with Start's value if necessary
                if start_end[0] != start:
                    start_end[0] = start
                if start_end[1] != start:
                    start_end[1] = start

            # Perform the page update with the determined values
            notion.pages.update(page.id, properties={
                ...
                End_Notion_Name: {
                    'date': {
                        'start': end.isoformat() if end else None,
                    }
                },
                Date_Notion_Name: {
                    'date': {
                        'start': start_end[0].isoformat(),
                        'end': start_end[1].isoformat() if start_end[1] else None,
                    }
                }
            })
