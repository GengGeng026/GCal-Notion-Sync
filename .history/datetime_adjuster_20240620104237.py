        ...
        if end is None:
            ...
            if start is not None and start_end is not None:
                if StartEnd_to_Overwrite_All:
                    # Convert 'start_end', 'start', and 'end' to naive datetime objects to handle timezone information correctly
                    start_end_naive = [d.replace(tzinfo=None) if d.tzinfo is not None else d for d in start_end]
                    start_naive = start.replace(tzinfo=None) if start.tzinfo is not None else start

                    # Do not update end if it's None
                    if start_end_naive[0] != start_naive:
                        start = start_end_naive[0]
                        if start_end_naive[1] != start_naive:
                            end = start_end_naive[1]
