...
def update_previous_dates(page, start, end, start_end, as_date=False, keep_midnight=False):
    # Parse 'start', 'end', and 'start_end' from strings into datetime objects if they are strings
    start = parse(start) if isinstance(start, str) else start
    end = parse(end) if isinstance(end, str) else end
    if isinstance(start_end, str):
        start_end = parse(start_end)
    elif isinstance(start_end, dict):
        start_end = {k: parse(v) if isinstance(v, str) else v for k, v in start_end.items()}
    elif isinstance(start_end, tuple) and any(isinstance(date, str) for date in start_end):
        start_end = tuple(parse(date) if isinstance(date, str) else date for date in start_end)

    # Existing logic to convert 'start' and 'end' into datetime objects if they are not already
    if isinstance(start, date) and not isinstance(start, datetime):
        start = datetime.combine(start, datetime.min.time())
    if end is not None and isinstance(end, date) and not isinstance(end, datetime):
        end = datetime.combine(end, datetime.min.time())

    # Existing logic to parse 'start' and 'end' values of start_end into datetime objects
    # Adjusted to handle the case where start_end is already parsed above
    if isinstance(start_end, dict):
        start_end = [start_end['start'], start_end['end']]
        ...
        if all(isinstance(date, datetime) for date in start_end):
            ...
            for date in start_end:
                temp_start_end.append(date if isinstance(date, datetime) else None)
            ...
            if StartEnd_to_Overwrite_All == True:
                ...
                if start_end_prop:  # Check if start_end_prop is not None and not empty
                    ...
                    if start_end != start:
                        ...
                        with lock:
                            start_end_list = []
                            end_date = None  # Initialize end_date to None or a suitable default value
                            ...
                            # 決定是否更新 'end' 或保持為 None
                            if 'end' in start_end_prop and start_end_prop.get('end') is not None:
                                end_date = start_end_prop['end']
                                if not isinstance(end_date, datetime):
                                    end_date = parse(end_date)
                                # 這裡不再自動添加 end_date 到 start_end_list，僅在確定需要更新時添加

                            # 如果 'end' 不在 start_end_prop 中，表示我們可能希望保持其為 None 或不變
                            # 如果您希望在不存在時明確設置為 None，請取消註釋下一行
                            # start_end_list.append(None)
                            # 這裡不做任何操作，意味著 end_date 根據之前的邏輯保持不變或為 None

                            # 只有在需要更新 End 時才將其添加到列表中
