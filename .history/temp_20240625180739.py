...
class ProgressThread(threading.Thread):
    ...
    def update_progress(self, size, progress_thread, updates, sleep_time=0.02):
        for _ in range(updates):
            # 更新进度条之前，先清屏并重新打印动画文本
            print("\r\033[K", end="")  # 清除当前行
            animate_text_wave("current_phase", repeat=1)  # 重绘动画文本，repeat=1 仅为示例
            progress_thread.update_progress(0.1)  # 更新进度条
            time.sleep(sleep_time)
