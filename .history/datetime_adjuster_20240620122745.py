...
def update_page_properties(notion, page, Start_Notion_Name, End_Notion_Name, Date_Notion_Name, start, end, start_end, as_date=False, single_date=False, update_all=True, keep_midnight=False, remove_start_end_midnight=False, StartEnd_to_Overwrite_All=False):
    ...
    if update_all or StartEnd_to_Overwrite_All:
        ...
        # For StartEnd, only use start, do not automatically add end
        properties[Date_Notion_Name] = {
            'date': {
                'start': start.isoformat(),
                'end': None  # Ensure end is always None
            }
        }
    else:
                ...
                if start_end_prop:  # Check if start_end_prop is not None and not empty
                    ...
                    if start_end != start:
                        ...
                        with lock:
                            ...
                            # Keep the original end date as None
                            start_end_list.append(None)  # Ensure 'end' remains None

                            update_page_properties(notion, local_data.page, 'Start', 'End', 'StartEnd', start, None, start_end_list)  # Pass None for end

                        # Update the 'Previous Start' and 'Previous End' properties
                        with lock:
                            update_previous_dates(local_data.page, start, None, start_end)  # Pass None for end

                        # Update the page object
                        with lock:
                            update_page(local_data.page, start, None, start_end_prop)  # Pass None for end

                        # Update the 'result' dictionary
                        with lock:
                            result = update_result(result, local_data.page, page_title, original_start, original_end, start, None, start_end_prop, prev_start, prev_end, original_start, original_end, original_start_end, updated_start_end)
