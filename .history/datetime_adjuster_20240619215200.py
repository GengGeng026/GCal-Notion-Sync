    ...
    if end is None:
        ...
        if start is not None and start_end is not None:
            ...
            if StartEnd_to_Overwrite_All == True:
                ...
                # 在判断 start_end_naive != start_naive 之前
                if not isinstance(start, datetime):
                    start = parse(start)  # 只有当 start 不是 datetime 对象时才执行
                    ...
                    if start_end != start:
                        ...
                        with lock:
                            ...
                            if start_end_prop.get('start') is not None:
                                start_end_list.append(parse(start))  # Update only the start date
                            else:
                                start_end_list.append(None)  # Default value assignment

                            if start_end_prop.get('end') is not None:
                                start_end_list.append(parse(start_end_prop['end']))  # Keep the original end date
                            else:
                                start_end_list.append(None)  # Default value assignment
