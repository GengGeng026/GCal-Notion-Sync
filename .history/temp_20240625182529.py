...
def animate_text_wave(text, repeat=1, sleep_time=0.01, continue_animation=lambda: False):
    length = len(text)
    animation_chars = ['/', '-', '\\', '|']
    while continue_animation():  # 直接使用 continue_animation 作为循环条件
        for _ in range(repeat):
            for i in range(int(length + 2) * 2):  # 优化迭代控制，直接使用整数迭代
                wave_text = []
                for j in range(length):
                    char = text[j].upper() if j >= i / 2 - 1 and j < i / 2 + 2 else text[j].lower()
                    wave_text.append(char)

                current_animation_char = animation_chars[i // 2 % len(animation_chars)]
                wave_text.append(" " + current_animation_char)

                sys.stdout.write(f"\r{''.join(wave_text)}")  # 使用 join 连接字符串
                sys.stdout.flush()
