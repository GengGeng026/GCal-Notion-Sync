...
# 保留您原有的函数定义不变
def animate_text_wave(text, repeat=1, sleep_time=0.01, continue_animation=lambda: False):  # 调整睡眠时间以减慢动画速度
    length = len(text)
...
for phase, updates in phases:
    animate_text_wave("phase", repeat=6, sleep_time=0.01, continue_animation=lambda: False)
