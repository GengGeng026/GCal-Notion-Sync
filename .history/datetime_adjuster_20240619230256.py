        ...
        if start is not None and start_end is not None:
            ...
            if StartEnd_to_Overwrite_All == True:
                ...
                if start_end_prop:  # Check if start_end_prop is not None and not empty
                    ...
                    with lock:
                        # 假设 start_end_prop 已经定义
                        start_end_list = []
                        end_date = None  # 明确设置 end_date 为 None

                        # 检查 'start' 是否存在，并进行解析
                        if start_end_prop.get('start') is not None:
                            if not isinstance(start, datetime):
                                start = parse(start)
                            start_end_list.append(start)  # 更新 start 日期

                        # 不需要检查或更新 end_date，因为我们希望它保持为 None

                        # 更新页面属性，注意这里 end_date 始终为 None
