...
def update_page_properties(notion, page, Start_Notion_Name, End_Notion_Name, Date_Notion_Name, start, end, start_end, as_date=False, single_date=False, update_all=True, keep_midnight=False, remove_start_end_midnight=False):
    # 如果 end 为 None，且没有明确的 start_end 覆盖，才将 end 设置为 start 的值
    # 这个检查已经足够，后面的代码不应该修改 end 的值，除非在特定条件下
    if end is None and start is not None and (start_end is None or start_end == (None, None)):
        end = start
    ...
    if isinstance(start_end, (list, tuple)) and len(start_end) == 2:
        ...
        # 只有当 end 为 None 时才考虑修改 start_end[1]
        if start_end[1] is None and end is None:
            start_end[1] = start_end[0]
    # 如果 start 不为 None，且 start_end 为 None 或者为 (None, None)，则将 start 赋值给 start_end
    if start is not None and (start_end is None or start_end == (None, None)):
