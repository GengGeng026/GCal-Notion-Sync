...
def update_page_properties(notion, page, Start_Notion_Name, End_Notion_Name, Date_Notion_Name, start, end, start_end, as_date=False, single_date=False, update_all=True, keep_midnight=False, remove_start_end_midnight=False):
    ...
    # Only convert 'end' if it's not None and not already a datetime object
    if end is not None and isinstance(end, date) and not isinstance(end, datetime):
        end = datetime.combine(end, datetime.min.time())

    # If end is None, do not assign it the value of start
    # This is the key change to ensure 'end' remains None if it's None
    ...
    if update_all:
        ...
        if start_end is not None and len(start_end) == 2:
            properties[Date_Notion_Name] = {
                'date': {
                    'start': start_end[0].isoformat(),
                    'end': start_end[1].isoformat() if start_end[1] is not None else None
                }
            }
    else:
    ...
    # Make the API request to update the page properties
