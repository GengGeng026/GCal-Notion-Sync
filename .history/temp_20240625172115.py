...
class ProgressThread(threading.Thread):
    ...
    def run(self):
        while not self.stop_event.is_set():
            ...
            bar_length = 50  # 進度條的長度
            filled_length = int(bar_length * self.downloaded_size // self.total_size)
            # 根據填充長度計算顏色（從熒光綠到黃色）
            green_length = int(filled_length * (1 - percent_complete / 100))
            yellow_length = filled_length - green_length
            bar = '\x1b[92m' + '━' * green_length + '\x1b[93m' + '━' * yellow_length + '\x1b[0m'
