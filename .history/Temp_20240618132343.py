...
# Set up logging
# åœ¨æ­£å¼éƒ¨ç½²å‰ï¼Œå°‡æ—¥èªŒç´šåˆ¥è¨­ç½®ç‚º INFO æˆ–æ›´é«˜ï¼Œé€™æ¨£å¯ä»¥æ¸›å°‘ç´°ç¯€çš„è¼¸å‡º
...
def obtain_calendar(service):
    try:
        ...
        # åœ¨ç™¼ç”ŸéŒ¯èª¤æ™‚ï¼Œé‡æ–°æŽˆæ¬Šä¸¦é‡æ–°åˆå§‹åŒ–æœå‹™
        credentials = refresh_token()
        service = build("calendar", "v3", credentials=credentials)
        try:
            calendar = service.calendars().get(calendarId=DEFAULT_CALENDAR_ID).execute()
        except Exception as e:
            # å¦‚æžœä»ç„¶ç„¡æ³•ç²å–æ—¥æ›†ï¼Œå‰‡è¨˜éŒ„éŒ¯èª¤ä¸¦è¿”å›ž None
...
# Section A
# Filter Only Pages where 'to Auto-Syncâ€™ is Checked and within current month
...
def process_pages_condition_A(page, counts, details, lock, processed_pages, return_values):
    ...
    # MASTER CONDITION B :  â€˜Endâ€™ is always None
    if end is None:
    ...
    # MASTER CONDITION C :  â€˜Startâ€™ is always None
    if start is None:
    ...
    # MASTER CONDITION D :  â€˜Startâ€™, Endâ€™ and â€˜StarEndâ€™ Date Property are ALL PRESENT
    # OVERWRITE
...
with ThreadPoolExecutor(max_workers=3) as executor:
    ...
    for details in default_time_range_details:
        ...
        print(f"{details[3]} {formatted_colon}  {formatted_plain_none if start_end_value[0] is None and start_end_value[1] is None else f'{DateTimeIntoNotionFormat(start_end_value[0])} â€” {DateTimeIntoNotionFormat(start_end_value[1])}'} {formatted_right_arrow} {DateTimeIntoNotionFormat(future_dict['start_end'][0])} â€” {TimeIntoNotionFormat(future_dict['start_end'][1], future_dict['start_end'][0])}\n")
    ...
    for details in auto_default_setting_details:
        ...
        if start_end is not None and future_dict['start_end'] is not None:
            print(f"{formatted_startend} {formatted_colon}  {formatted_plain_none} {formatted_right_arrow} {DateTimeIntoNotionFormat(start, plus_time=True, time_format='24')} â€” {end_time_formatted}\n")
    ...
    for details in alldayevent_details:
        ...
        if start_date_formatted == end_date_formatted:
            ...
            print(f"{formatted_startend} {formatted_colon}  {formatted_plain_previous} {formatted_right_arrow} {start_date_formatted} â€” {end_date_formatted}\n")
    ...
    def process_pages_filled_details(pages_filled_details):
        ...
        for updated_details in pages_filled_details:
            ...
            print(f"{formatted_startend} {formatted_colon}  {formatted_plain_none + '  ' if extra is None else ''}{start_str + ' ' if original_start_str is not None and original_start_str == extra else ''}{'â€” ' + DateTimeIntoNotionFormat(original_end_str, time_format='24') if original_end_str is not None and original_end_str == original_start_str else ''}{formatted_right_arrow + '  ' if extra is None else ''}{start_str}  {'â€”  ' + (end_str) if end_str != start_str else ''}\n")
        none_field = ""
        present_field = ""
    ...
    def process_pages_overwritten_details(pages_overwritten_details):
        ...
        for details in pages_overwritten_details:
            ...
            startend_string += f"  {DateTimeIntoNotionFormat(start_end[0], date_only=False, plus_time=True, show_midnight=True, time_format='24')}  â€”  {end_date_formatted}\n"
