...
def update_page_properties(notion, page, Start_Notion_Name, End_Notion_Name, Date_Notion_Name, start, end, start_end, as_date=False, single_date=False, update_all=True, keep_midnight=False, remove_start_end_midnight=False, StartEnd_to_Overwrite_All=False):
    ...
    if not isinstance(start, datetime):
        print(f"Error: start is not a datetime object. It is: {start}")
        return

    if end is not None and not isinstance(end, datetime):
        print(f"Error: end is not a datetime object. It is: {end}")
        return
    ...
    # Adjusted snippet from update_page_properties function
    properties = {}
    if update_all or StartEnd_to_Overwrite_All:
        properties[Start_Notion_Name] = {
            'date': {
                'start': format_date_time(start, keep_midnight=True)
            }
        }
        if end is not None:
            properties[End_Notion_Name] = {
                'date': {
                    'start': format_date_time(start, keep_midnight=True),  # Adjust if Notion API requires a start date for End property
                    'end': format_date_time(end, keep_midnight=True)
                }
            }
        properties[Date_Notion_Name] = {
            'date': {
                'start': format_date_time(start, keep_midnight=True),
            }
        }
        ...
        if end is not None:
            properties[End_Notion_Name] = {
                'date': {
                    'end': format_date_time(end, keep_midnight=True)  # Ensure this is correctly setting only the 'end' date
                }
            }

    update_payload = {"page_id": page['id'], "properties": properties}
    notion.pages.update(**update_payload)
    ...
    if end is not None:
        ...
        keep_end_midnight = False  # 或根据逻辑需要选择合适的默认值
    ...
    if update_all:
        ...
        # Check if end_date is not None before updating End_Notion_Name
...
def update_previous_dates(page, start, end, start_end, original_end=None, as_date=False, keep_midnight=False):
    ...
    if isinstance(start, str):
        ...
        start = datetime.combine(start, datetime.min.time()).replace(tzinfo=pytz.UTC)  # Default to UTC if no timezone
    
    if end is not None:
        if isinstance(end, str):
            end = parse(end)
        elif isinstance(end, date) and not isinstance(end, datetime):
            end = datetime.combine(end, datetime.min.time()).replace(tzinfo=pytz.UTC)  # Default to UTC if no timezone

    # Handle conversion of 'start_end'
    if isinstance(start_end, dict):
        start_end = [parse(start_end['start']), parse(start_end.get('end', start_end['start']))]
    elif isinstance(start_end, (tuple, list)):
        start_end = [parse(date) if isinstance(date, str) else date for date in start_end]
    elif isinstance(start_end, datetime):  # Handle case where start_end is a single datetime object
        start_end = [start_end, start_end]

    # New logic to handle 'Previous End' based on 'original_end'
    if original_end is None:
        previous_end = None
    else:
        previous_end = original_end  # Implement your logic here

    # Check if start, end, and start_end are datetime objects
    if not all(isinstance(date, datetime) for date in [start, end] if date is not None) or not isinstance(start_end, (list, tuple)) or len(start_end) != 2 or not all(isinstance(date, datetime) for date in start_end):
        print(f"Error: start, end, or start_end is not a datetime object. They are: {type(start)}, {type(end)}, {[type(date) for date in start_end]}")
        return
    ...
    # Check if only start or end is overwritten by start_end_start or start_end_end respectively
    keep_start_midnight = start.time() != time(0, 0) or (start.time() == time(0, 0) and end is not None and end.time() != time(0, 0))
    keep_end_midnight = end is not None and (end.time() != time(0, 0) or (end.time() == time(0, 0) and start.time() != time(0, 0)))
    ...
    # Initialize properties dictionary with 'Previous Start'
    properties = {
        'Previous Start': {
            'date': {
                'start': start_str,
    ...
    # Only include 'Previous End' if end_str is not None
    if end_str is not None:
        properties['Previous End'] = {
            'date': {
                'start': end_str,
            }
        }
    ...
    # Check if start and end are not None, are different dates, and either start or end is not midnight, and start_end is a different time range
    if start is not None and end is not None and start.date() != end.date() and (start.time() != datetime.min.time() or end.time() != datetime.min.time()):
...
with ThreadPoolExecutor(max_workers=3) as executor:
    ...
    def process_pages_filled_details(pages_filled_details):
        ...
        for updated_details in pages_filled_details:
            ...
            # Simplified and adjusted for consistent spacing
            print(f"{formatted_start}    {formatted_colon}  {' ' + formatted_plain_none + '  ' if original_start is None else DateTimeIntoNotionFormat(start_new, date_only=False, time_format='24') + ' ' if original_start is not None else ''}{formatted_right_arrow if original_start is None and original_end is not None or original_start is None and original_end is None and extra is not None else ''}{'  ' + formatted_plain_none if start_new is None else DateTimeIntoNotionFormat(start_new, date_only=False, time_format='24') if start_new != original_start else ''}")
            print(f"{formatted_end}      {formatted_colon}  {' ' + formatted_plain_none + '  ' if original_end is None else DateTimeIntoNotionFormat(end_new, date_only=False, time_format='24') + ' ' if original_end is not None else ''}{formatted_right_arrow if original_end is None and original_start is not None or original_end is None and original_start is None and extra is not None else ''}{'  ' + formatted_plain_none if end_new is None else DateTimeIntoNotionFormat(end_new, date_only=False, time_format='24') if end_new != original_end and start_new != end_new or original_end == original_start or original_end is None else ''}")
