...
with ThreadPoolExecutor(max_workers=3) as executor:
    ...
    def process_pages_filled_details(pages_filled_details):
        ...
        for updated_details in pages_filled_details:
            ...
            print(f"{formatted_start}    {formatted_colon}  {' ' + formatted_plain_none + ' ' if original_start is None else DateTimeIntoNotionFormat(start_new, date_only=False, time_format='24') + ' ' if original_start is not None else ''}{'▸' if original_start is None and original_end is not None or original_start is None and original_end is None and extra is not None else ''}{' ' + formatted_plain_none if start_new is None else DateTimeIntoNotionFormat(start_new, date_only=False, time_format='24') if start_new != original_start else ''}")
            print(f"{formatted_end}      {formatted_colon}  {' ' + formatted_plain_none + ' ' if original_end is None else DateTimeIntoNotionFormat(end_new, date_only=False, time_format='24') + ' ' if original_end is not None else ''}{'▸' if original_end is None and original_start is not None or original_end is None and original_start is None and extra is not None else ''}{' ' + formatted_plain_none if end_new is None else DateTimeIntoNotionFormat(end_new, date_only=False, time_format='24') if end_new != original_end and start_new != end_new or original_end == original_start or original_end is None else ''}")
