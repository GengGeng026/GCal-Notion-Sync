...
class ProgressThread(threading.Thread):
    def __init__(self, total_size):
        ...
        self.downloaded_size = 0.0  # Use float for more granular updates
...
# 在后台更新进度的函数
def update_progress_in_background(progress_thread, update_amount, total_updates, delay):
    for _ in range(total_updates):
        progress_thread.update_progress(update_amount)
        time.sleep(delay)
    progress_thread.stop()

# 创建后台线程来更新进度
def start_progress_thread(progress_thread):
    # Part 1
    animate_text_wave("initializing", repeat=5)
    threading.Thread(target=update_progress_in_background, args=(progress_thread, 0.1, 10, 0.02)).start()

    # Part 2
    animate_text_wave("checking", repeat=5)
    threading.Thread(target=update_progress_in_background, args=(progress_thread, 0.1, 20, 0.02)).start()

    # Part 3
    animate_text_wave("modifying", repeat=5)
    threading.Thread(target=update_progress_in_background, args=(progress_thread, 0.1, 30, 0.02)).start()

    # Part 4
    animate_text_wave("updating", repeat=5)
    threading.Thread(target=update_progress_in_background, args=(progress_thread, 0.1, 40, 0.02)).start()

    # Part 5
    animate_text_wave("finishing", repeat=5)
    threading.Thread(target=update_progress_in_background, args=(progress_thread, 0.1, 50, 0.02)).start()

# 示例使用
total_size = 100.0  # 假设总大小为100
progress_thread = ProgressThread(total_size)
progress_thread.start()

# 使用start_progress_thread函数启动进度更新
start_progress_thread(progress_thread)