...
def update_page_properties(notion, page, Start_Notion_Name, End_Notion_Name, Date_Notion_Name, start, end, start_end, as_date=False, single_date=False, update_all=True, keep_midnight=False, remove_start_end_midnight=False):
    # Assuming start, end, and start_end are defined somewhere above in your code
    # and they are in the format "2024-06-19 14:00:00+08:00" as strings.
    ...
    start_end = [parse(d) for d in start_end] if isinstance(start_end, (list, tuple)) and all(isinstance(d, str) for d in start_end) else start_end
...
ddef update_result(result, page, page_title, original_start, original_end, start, end, start_end_prop, prev_start, prev_end, original_start_end, updated_start_end):
    # Adjust the function to correctly handle the parameters it's supposed to accept.
    # Assuming 'start_end' was meant to be 'start_end_prop', and 'prev_start_value', 'prev_end_value', 'new_start_value', 'new_end_value'
    # are derived from 'prev_start', 'prev_end', 'start', and 'end' respectively, but were not passed as arguments.
    # This adjustment is based on the provided context and may need further refinement.

    # Example conversion, assuming 'format_date_time' is a function that formats datetime objects to strings.
    prev_start_value = format_date_time(prev_start) if prev_start else None
    prev_end_value = format_date_time(prev_end) if prev_end else None
    new_start_value = format_date_time(start) if start else None
    new_end_value = format_date_time(end) if end else None
    ...
    result.update({
        ...
        'start': start,
        'end': end,
        'start_end': start_end_prop,  # Assuming this was the intended parameter
        ...
        'original_start_end': original_start_end,
        'updated_start_end': updated_start_end
    })
    return result
...
def get_current_end():
    # 这里应该是获取当前End属性值的逻辑
    # 例如: return local_data.page['properties']['End']['date']
    return None  # 返回一个示例值或者从实际数据源获取

# 定义check_if_end_needs_update函数
def check_if_end_needs_update(start_end_prop, current_end):
    """
    检查是否需要更新End属性。
    :param start_end_prop: 包含StartEnd属性新值的字典。
    :param current_end: 当前End属性的值。
    :return: 如果需要更新End，则返回True；否则返回False。
    """
    # 如果start_end_prop中没有'end'，或者'end'值为None，则不需要更新
    if 'end' not in start_end_prop or start_end_prop.get('end') is None:
        return False
    
    # 解析start_end_prop中的'end'值
    new_end = start_end_prop['end']
    if not isinstance(new_end, datetime):
        new_end = parse(new_end)
    
    # 如果current_end为None，或者new_end与current_end不相等，则需要更新
...
# 获取当前End的值
current_end = get_current_end()


# Assuming 'is_datetime', 'parse_datetime', and 'ensure_datetime' are already defined
# Here are placeholders for these functions for completeness
def is_datetime(obj):
    return isinstance(obj, datetime)

def parse_datetime(date_str):
    # Placeholder: Parse string to datetime object
    try:
...
def ensure_datetime(obj):
    if obj is None or is_datetime(obj):
        return obj
    try:
...
# Function to update page properties with start and optional end date
def update_page_properties_with_start_and_optional_end(start, end):
    # Placeholder: Logic to update page properties
    print(f"Updated page properties with start: {start} and end: {end}")

# Function to update the result dictionary accordingly
def update_result_dictionary_accordingly():
    # Placeholder: Logic to update result dictionary
    print("Updated result dictionary accordingly")

# Test scenario functions
def test_scenario_with_both_start_and_end():
    print("Testing scenario with both start and end")

def test_scenario_with_only_start():
    print("Testing scenario with only start")

def test_scenario_with_neither_start_nor_end():
    print("Testing scenario with neither start nor end")

# Function to update previous dates
def update_previous_dates(page, start, end, start_end):
    # Placeholder: Logic to update previous dates
    print(f"Updated previous dates with start: {start}, end: {end}, and start_end: {start_end}")

# Function to update the page object
def update_page(page, start, end, start_end_prop):
    # Placeholder: Logic to update the page object
    print(f"Updated page object with start: {start}, end: {end}, and start_end_prop: {start_end_prop}")

# Function to update the result dictionary
def update_result(result, page, page_title, original_start, original_end, start, end, start_end_prop, prev_start, prev_end, original_start_end, updated_start_end):
    # Placeholder: Logic to update the result dictionary
    print("Updated result dictionary with new values")
    return result

# Function to calculate total pages modified
def calculate_total(counts):
    # Placeholder: Logic to calculate total
    return counts.get('count_pages_filled', 0)
    ...
    if end is None:
        ...
        if start is not None and start_end is not None:
            ...
            if StartEnd_to_Overwrite_All == True:
                ...
                # Example usage in your existing logic
                if not is_datetime(start) or (end is not None and not is_datetime(end)) or not is_datetime(start_end):
                    print(f"Error: start, end, or start_end is not a datetime object. They are: {start}, {end}, {start_end}")
                else:
                    updated_start_end = None  # or some default value appropriate to your logic
                    start = ensure_datetime(start)
                    end = ensure_datetime(end)  # Since end can be None, ensure_datetime should handle None gracefully
                    start_end = ensure_datetime(start_end)
                    start_end_prop = local_data.page['properties']['StartEnd']['date']
                                
                    print(f"Type of start: {type(start)}, Type of end: {type(end)}, Type of start_end: {type(start_end)}")    

                    print(f"start_end_prop: {start_end_prop}")
                    if start_end_prop:  # Check if start_end_prop is not None and not empty
                        ...
                        print(f"original_start_end: {original_start_end}")

                        with lock:
                            start_end_list = []
                            # Initialize end_date to None and update it if 'end' exists in start_end_prop
                            end_date = None

                        # Adjust parsing logic to handle None for end dates correctly
                        if start_end_prop.get('start') is not None:
                            start = parse_datetime(start_end_prop['start'])
                        if start_end_prop.get('end') is not None:
                            end = parse_datetime(start_end_prop['end'])
                        else:
                            end = None  # Explicitly handle None end dates

                        # Update page properties and result dictionary with refined logic
                        if start and (end is None or is_datetime(end)):
                            update_page_properties_with_start_and_optional_end(start, end)
                            update_result_dictionary_accordingly()

                        # Test all scenarios to ensure logic behaves as expected
                        test_scenario_with_both_start_and_end()
                        test_scenario_with_only_start()
                        test_scenario_with_neither_start_nor_end()

                        # Update the 'Previous Start' and 'Previous End' properties
                        with lock:
                            print(f"Updating previous dates with start: {start} and end: {end}")
                            update_previous_dates(local_data.page, start, end, start_end)

                        # Update the page object
                        with lock:
                            print(f"Updating page object with start: {start}, end: {end}, and start_end_prop: {start_end_prop}")
                            update_page(local_data.page, start, end, start_end_prop)
                        ...
                        with lock:
                            counts['count_pages_filled'] += 1
                            result['total_pages_modified'] = calculate_total(counts)
                            print(f"Page count updated: {counts['count_pages_filled']}")

                        # Only add details to the list if 'StartEnd' was None before the update
                        with lock:
                            print("Adding details to result['details']['pages_filled_details']")
                            result['details']['pages_filled_details'][result['page_id']] = (result['page_title'], result['start'], result['end'], result['prev_start_value'], result['prev_end_value'], result['start_end'], updated_start_end)
