...
# Ensure is_datetime function accurately validates datetime objects
...
def ensure_datetime(date_str_or_datetime):
    if date_str_or_datetime is None:
        return None  # or return a default datetime value if needed
    elif isinstance(date_str_or_datetime, datetime):
        return date_str_or_datetime  # Return the datetime object directly if it's already a datetime
    else:
        return parse(date_str_or_datetime)
                ...
                if not is_datetime(start) or (end is not None and not is_datetime(end)) or not is_datetime(start_end):
                    ...
                    if start_end_prop:  # Check if start_end_prop is not None and not empty
                        ...
                        with lock:
                            ...
                            # Check if 'start' exists and parse it
                            if start_end_prop.get('start') is not None:
                                if not isinstance(start, datetime):
                                    start = parse(start)
                                start_end_list.append(start)  # Update start date
                                print(f"start parsed and added to start_end_list: {start}")

                            # Check if 'end' exists and parse it
                            if start_end_prop.get('end') is not None:
                                if not isinstance(end, datetime):
                                    end = parse(end)
                                end_date = end  # Update end date
                                start_end_list.append(end)  # Update end date in list
                                print(f"end parsed and added to start_end_list: {end}")

                            # Update page properties, now considering end_date
                            print(f"Updating page properties with start: {start} and end_date: {end_date}")
                            update_page_properties(notion, local_data.page, 'Start', 'End', 'StartEnd', start, end_date, start_end_list)
