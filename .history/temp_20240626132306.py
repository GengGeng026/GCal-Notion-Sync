...
class ProgressThread(threading.Thread):
    ...
    def update_progress(self, size, phase_text=""):
        # 清除旧输出并打印动画文本
        sys.stdout.write("\033[2K")  # 清除当前行
        sys.stdout.write(f"\033[1;0H{phase_text}\n")  # 在第一行打印动画文本
        self.downloaded_size += size
        percent_complete = (self.downloaded_size / self.total_size) * 100
        bar_length = 50
        filled_length = int(bar_length * self.downloaded_size // self.total_size)
        bar = "━" * filled_length + " " * (bar_length - filled_length)
        sys.stdout.write(f'\r   {bar} {percent_complete:.1f}%\n')  # 在第二行更新进度条
        sys.stdout.flush()
        if self.downloaded_size >= self.total_size:
            self.stop()
...
def update_progress(size, progress_thread, updates, sleep_time=0.01, phase_text=""):
...
for i in range(10):  # 1 is divided into 10 updates of 0.1 each
    progress_thread.update_progress(0.1)
    time.sleep(0.02)

print("\r\033[K", end="")  # Clear the line after the progress bar

# Part 2
animate_text_wave("checking", repeat=5)
for i in range(20):  # 2 is divided into 20 updates of 0.1 each
    progress_thread.update_progress(0.1)
    time.sleep(0.02)

print(f"some code is running here")

# Part 3
animate_text_wave("modifying", repeat=5)
for i in range(30):  # 3 is divided into 30 updates of 0.1 each
    progress_thread.update_progress(0.1)
    time.sleep(0.02)
...
for i in range(40):  # 4 is divided into 40 updates of 0.1 each
    progress_thread.update_progress(0.1)
    time.sleep(0.02)
...
for i in range(50):  # 5 is divided into 50 updates of 0.1 each
    progress_thread.update_progress(0.1)
    time.sleep(0.02)
