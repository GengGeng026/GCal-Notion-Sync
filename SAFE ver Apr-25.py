...
NOTION_TOKEN = "secret_7RpFWckv1znIT03lbNh6wTlK6vrdRjL8CAEO6FdrqUJ" #the secret_something from Notion Integration

# Create an instance of the Notion client
notion = Client(auth=NOTION_TOKEN)

database_id = "d99f99fce67946de99657100c833a4c7"#get the mess of numbers before the "?" on your dashboard URL (no need to split into dashes)

urlRoot = 'https://www.notion.so/geng-is-here/d99f99fce67946de99657100c833a4c7?v=22f872345e61427d80ef96c3051ec704&p=' #open up a task and then copy the URL root up to the "p="
            
runScript = "python3 /Users/mac/Documents/pythonProjects/Notion-and-Google-Calendar-2-Way-Sync-main/GCalToken.py" #This is the command you will be feeding into the command prompt to run the GCalToken program

#GCal Set Up Part

credentialsLocation = "/Users/mac/Documents/pythonProjects/Notion-and-Google-Calendar-2-Way-Sync-main/token.pkl" #This is where you keep the pickle file that has the Google Calendar Credentials
...
DEFAULT_CALENDAR_ID = 'danielngkahking@gmail.com' #The GCal calendar id. The format is something like "sldkjfliksedjgodsfhgshglsj@group.calendar.google.com"
...
calendarDictionary = {
    ...
    'Life' : 'danielngkahking@gmail.com',  #just typed some random ids but put the one for your calendars here
    'Soka': 'h9p2chb00p6mq2n8ggu4p9citk@group.calendar.google.com',
    'Work': 'cf67e39e9038433768df2a22b4680161112f4396e60e53da105d1d1b0c3a2e7f@group.calendar.google.com'
}
...
# Constants
CREDENTIALS_LOCATION = '/Users/mac/Documents/pythonProjects/Notion-and-Google-Calendar-2-Way-Sync-main/token.pickle'
CLIENT_SECRET_FILE = '/Users/mac/Documents/pythonProjects/Notion-and-Google-Calendar-2-Way-Sync-main/client_secret.json'
                    ...
                    if StartEnd_to_Overwrite_All == True:
                        ...
                        if start_end_prop:
                            ...
                            if start_end_prop['end'] is not None:
                                ...
                                new_end = None  # or any default value you want to assign
                        
                        # Check if the start and end dates have a time component
                        if new_start is not None or new_end is None:
                            if new_start.time() == dt_time(0, 0):
                                # If they do, replace the timezone information with the specified timezone
                                new_start = new_start.replace(tzinfo=None) if new_end == None else new_start
                        ...
                        if end is None and start_end is not None:
                            end = start_end.date()
...
with ThreadPoolExecutor(max_workers=3) as executor:
    ...
    def process_pages_filled_details(pages_filled_details):
        ...
        for updated_details in pages_filled_details:
            ...
            print(f"\noriginal_start: {original_start}, type: {type(original_start)}")
            print(f"start: {start}, type: {type(start)}")
            print(f"start_new: {start_new}, type: {type(start_new)}")
            print(f"original_end: {original_end}, type: {type(original_end)}")
            print(f"end: {end}, type: {type(end)}")
            print(f"end_new: {end_new}, type: {type(end_new)}")
            print(f"extra: {extra}, type: {type(extra)}")
            print(f"updated_start_end: {updated_start_end}, type: {type(updated_start_end)}\n")
